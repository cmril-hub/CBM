...
def main():
    ...
    with tabs[0]:
        ...
        TCIA_TOKEN_URL = "https://services.cancerimagingarchive.net/nbia-api/oauth/token"
            ...
            if "TCIA" in mode:
                ...
                def _tcia_series_list():
                    ...
                    def _get_guest_token():
                        """Obtain a short-lived guest token from the NBIA OAuth endpoint."""
                        try:
                            resp = _req.post(
                                TCIA_TOKEN_URL,
                                data={
                    ...
                    def _safe_json(resp):
                        """Parse JSON safely; return descriptive error dict on failure."""
                        text = (resp.text or "").strip()
                        if not text:
                            return {"error": (
                                f"API returned an empty response "
                                f"(HTTP {resp.status_code}). "
                                "The TCIA service may be temporarily down or "
                        ...
                        if "html" in ct.lower():
                            return {"error": (
                                ...
                                f"(HTTP {resp.status_code}). "
                                "This usually means the endpoint redirected to a "
                    ...
                    headers = {}
                    # Try unauthenticated first; fall back to guest token if needed.
                    for attempt in range(2):
                        try:
                            ...
                            # A 401/403 on attempt 0 → retry with token
                            if attempt == 0 and r.status_code in (401, 403):
                                continue
                            r.raise_for_status()
                            result = _safe_json(r)
                            if isinstance(result, dict) and "error" in result and attempt == 0:
                                # Empty/bad body on unauthenticated → retry with token
                                continue
                            return result
                        except _req.exceptions.ConnectionError:
                            return {"error": "Connection refused — check your internet connection."}
                        except _req.exceptions.Timeout:
                            return {"error": "Request timed out after 30 s. TCIA may be slow; try again."}
                        except _req.exceptions.HTTPError as he:
                            if attempt == 0 and he.response is not None and \
                                    he.response.status_code in (401, 403):
                                continue  # retry with guest token
                            return {"error": str(he)}
                        except Exception as e:
                            return {"error": str(e)}
                    return {"error": "Could not authenticate with TCIA after two attempts."}
                ...
                if isinstance(series_data, dict) and "error" in series_data:
                    ...
                    if not patients_avail:
                        ...
                        if st.button(
                            ...
                            # Obtain a guest token for authenticated downloads
                            _dl_token = None
                            try:
                                _tok_r = _req.post(
                                    TCIA_TOKEN_URL,
                                ...
                                if _tok_r.ok and _tok_r.text.strip():
                                    _dl_token = _tok_r.json().get("access_token")
                            except Exception:
                                pass  # fall back to unauthenticated
                            _dl_headers = (
                                {"Authorization": f"Bearer {_dl_token}"}
                                if _dl_token else {}
                            )
                            ...
                            for idx, (pid, date, uid) in \
                                ...
                                try:
                                    ...
                                    if not raw:
                                        raise ValueError(
                                            "Empty response — series may require "
                                            "additional permissions or does not exist.")
