...
def main():
    ...
    with tabs[0]:
        ...
        mode = st.radio(
            ...
            ["📂 Local / server directory path  (recommended for large datasets)",
             "⬆️ Upload .rar or .zip  (≤ 600 MB)"],
            horizontal=True,
            help="Use the directory option for the full QIN dataset — "
                 "no upload needed, any size supported.")

        col_up, col_info = st.columns([3, 2])

        def _run_full_pipeline(extract_dir, status_ctx):
            """Steps 3 + 4: index + segment."""
            # Store base_dir so resolve_dcm_path works on any machine
            ...
            if dcm_index:
                ...
                # Prune cap-hit / noise results and clear display caches
        ...
        with col_up:
            if "📂" in mode:
                st.markdown("""
                <div style="background:#0D2D4E;border-radius:10px;padding:14px 18px;
                            border-left:4px solid #4DD4C4;margin-bottom:12px">
                <b style="color:#4DD4C4">Directory indexing — no upload required</b>
                <p style="color:#CDD4E0;font-size:13px;margin:6px 0 0 0">
                Paste the folder that contains your DICOM files.<br>
                Works for any dataset size. The app indexes the DICOM headers
                (fast) and only reads pixel data on demand.<br><br>
                <b>Local examples:</b><br>
                &nbsp;Windows: <code>C:\\Users\\HP\\Downloads\\QIN-Breast-DCE-MRI</code><br>
                &nbsp;Mac/Linux: <code>/home/user/data/QIN-Breast-DCE-MRI</code><br><br>
                <b>Web server / HuggingFace / Streamlit Cloud:</b><br>
                &nbsp;Pre-download the dataset to the server and enter its path, e.g.
                <code>/data/QIN-Breast-DCE-MRI</code>.<br>
                &nbsp;Paths are stored as <em>relative</em> paths in the index, so
                the same index file works wherever the dataset root lives.
                </p>
                </div>
                """, unsafe_allow_html=True)

                folder_default = st.session_state.get(
                    "last_folder",
                    r"C:\Users\HP\Downloads\QIN-Breast-DCE-MRI")
                folder = st.text_input(
                    "Dataset root folder",
                    value=folder_default,
                    placeholder=r"e.g. C:\Users\HP\Downloads\QIN-Breast-DCE-MRI",
                    help="The top-level folder containing all patient sub-folders.")
                ...
                <div class="upload-hint">
                    ...
                    (up to 600 MB — streamed to disk, segmentation runs automatically)
                </div>
                """, unsafe_allow_html=True)
                ...
                if uploaded:
                    if "dcm_index" not in st.session_state:
                        ...
                        with st.status("Processing archive …", expanded=True) as status:
                            ...
                            status.update(label="✅ Dataset ready! Segmentation complete.",
                                          state="complete")
